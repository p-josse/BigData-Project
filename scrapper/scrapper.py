...
def extract_car_info(soup):
    ad = {'Marque': None, 'Modele': None, 'Annee': None, 'Kilometrage': None, 'Transmission': None, 'Energie': None, 'Prix': None}
    ...
    prices = soup.find_all('span', class_='Text_Text_text Vehiculecard_Vehiculecard_price Text_Text_subtitle2')
    ...
    if len(all_titles)*4 == len(raw):
        ...
        for title in all_titles:
            ...
            clean_km = raw[index+1].replace("km", "").text.replace(" ", "")
            ad['Kilometrage'] = int(clean_km)
            ad['Transmission'] = raw[index+2].text.title()
            ad['Energie'] = raw[index+3].text.title()
            clean_price = prices[price_count].text.title().replace('€', '').replace(' ', '')
            ad['Prix'] = int(clean_price)
...
try:
    with open('/app/output/ads.json', 'w', encoding="utf-8") as json_file:
        json.dump(ads, json_file, indent=4, ensure_ascii=False)
    print("JSON data successfully written to 'ads.json'.")
